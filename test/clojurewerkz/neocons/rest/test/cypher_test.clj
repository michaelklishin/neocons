...
(deftest ^{:cypher true} test-cypher-query-example1
  (let [john  (nodes/create *connection* {:name "John"})
        sarah (nodes/create *connection* {:name "Sarah"})
        joe   (nodes/create *connection* {:name "Joe"})
        maria (nodes/create *connection* {:name "Maria"})
        steve (nodes/create *connection* {:name "Steve"})
        rel1  (rel/create *connection* john sarah :friend)
        rel2  (rel/create *connection* john joe :friend)
        rel3  (rel/create *connection* sarah maria :friend)
        rel4  (rel/create *connection* joe steve :friend)
        {:keys [data columns]} (cy/query *connection* "START john=node({sid}) MATCH john-[:friend]->()-[:friend]->fof RETURN john, fof" {:sid (:id john)})
...
(deftest ^{:cypher true} test-cypher-query-example2
  (let [john  (nodes/create *connection* {:name "John"})
        sarah (nodes/create *connection* {:name "Sarah"})
        rel1  (rel/create *connection* john sarah :friend)
        [row1] (cy/tquery *connection* "START x = node({sid}) MATCH path = (x--friend) RETURN path, friend.name" {:sid (:id john)})
        path   (instantiate-path-from (get row1 "path"))]
    (is (paths/included-in? *connection* john path))
    (is (paths/included-in? *connection* sarah path))
    (is (= "Sarah" (get row1 "friend.name")))))


(deftest ^{:cypher true} test-cypher-query-example3
  (let [john  (nodes/create *connection* { :name "John"  :age 27 })
        sarah (nodes/create *connection* { :name "Sarah" :age 28 })
        rel1  (rel/create *connection* john sarah :friend)
        ids   (map :id [john sarah])
        response (cy/tquery *connection* "START x = node({ids}) RETURN x.name, x.age" { :ids ids })]
...
(deftest ^{:cypher true} test-cypher-query-example4
  (let [john  (nodes/create *connection* { :name "John" })
        sarah (nodes/create *connection* { :name "Sarah" })
        ids   (map :id [john sarah])
        {:keys [data columns]} (cy/query *connection* "START x = node({ids}) RETURN x" {:ids ids})]
...
(deftest ^{:cypher true} test-cypher-query-example5
  (let [john  (nodes/create *connection* { :name "John" })
    ...
    (is (= ids (vec (map :id (nodes/get-many *connection* ids)))))))

(deftest ^{:cypher true} test-cypher-query-example6
  (let [john  (nodes/create *connection* { :name "John" })
        sarah (nodes/create *connection* { :name "Sarah" })
        tim   (nodes/create *connection* { :name "Tim" })
        rel1 (rel/create *connection* john sarah :friend)
        rel2 (rel/create *connection* sarah tim :friend)
        ids   (vec (map :id [rel1 rel2]))]
    (is (= ids (vec (map :id (rel/get-many *connection* ids)))))))
...
(deftest ^{:cypher true} test-cypher-tquery
  (let [john  (nodes/create *connection* { :name "John"  :age 27 })
    ...
    (is (= [{"x.name" "John"  "x.age" 27}
           ...
           (vec (cy/tquery *connection* "START x = node({ids}) RETURN x.name, x.age" { :ids ids }))))))
