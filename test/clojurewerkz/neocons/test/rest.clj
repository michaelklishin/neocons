...
(deftest test-cypher-query-example1
  (let [john  (nodes/create { :name "John" })
        ...
        nodes    (map instantiate-node-from (first (:data response)))]
    (is (= ["john" "fof"] (:columns response)))
    (is (= (:id john)    (:id (first nodes))))
    (is (= (:data john)  (:data (first nodes))))
    (is (= (:id maria)   (:id (last nodes))))
    (is (= (:data maria) (:data (last nodes))))))
